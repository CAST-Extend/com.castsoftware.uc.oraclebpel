...
class BpelExtension(cast.analysers.ua.Extension):
    ...
    def end_analysis(self):
        # Checking from the List (bpel_process_data) which contains here all BPEL file contents with Targetnamespace or name to match the WSDL file
        for child in bpel_process_data:
            ...
            # Checking from List (wsdl_file_data) which contains here all WSDL file with Targetnamespace or name to match the BPEL file    
            for ele in wsdl_file_data:
                ...
                #Matching if WSDL file and BPEL file Targetnamespace/name mateched or not if matched then creating link between these two files
        ...
        for child in bpel_invoke_data:
            ...
            for ele in bpel_invoke_data[child]:
                ...
                for subele in bpel_invoke_list:
                    CAST.debug("CHecking Invoke Port---- %s" +str(subele))
                ...
                #CAST.debug("Invoke Count %s" % invoke_count)
                ...
                if not("null" in port_type and "null" in operation_type):      
                    if ':' in port_type:
                        ...
                        #CAST.debug("Checking Port type----"+namespace_port)
                    #CAST.debug("Checking raw type----"+ele_data)   
                    ...
                    for invoke_data in list(ele_data.split(',')):
                        if namespace_port in invoke_data:
                            ...
                            #CAST.debug("Invoke Data----"+namespace_port + "--- Operation type---" + operation_type)       
                            break 
                    flag_link = 0
                    '''
                    CAST.debug(ele_data)
